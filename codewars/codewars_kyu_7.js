...
function removeDuplicateWords (s) {
    return s.split(' ').filter((el, index) => index === s.split(' ').findIndex(k => k === el)).join(' ')
}
...
function myLanguages(results) {
    ...
    let arrResults = Object.entries(results)
    let filteredResults = arrResults.filter(([lang, res]) => res >= 60)

    return filteredResults.sort((a, b) => b[1] - a[1]).map(([lang]) => lang)
}
...
function hasUniqueChars(str){
    // const res = str.split('').filter((el, index) => str.indexOf(el) !== index)
    // return res.length <= 0
